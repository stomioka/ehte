# #############################################################################
# Copyright 2023 Sumitomo Pharma America
#
...
class eHTE_Estimator:
    ...
    def calculate_sigma(self, indf_p, indf_t, st, en, pctiles, interval95):
        ...
        pct_trt = indf_t.copy()
        pct_trt['percentile'] = pct_trt.groupby(['NPERMS', 'TRT01PN'])['CHG'].apply(self.calculate_percentile)
    ...
    def cal_pvalue(self, simhte_df, ehte_df, s_1):
        ...
        for trt_ in list(sigmas.keys()):
            print('treatment: {}, sigma: {:2.5f}, eHTE: {:2.3f}, p-value: {:2.3f}'.format(self.trt_dict[int(trt_)], sigmas[trt_], sigma_stds[trt_],p_values[trt_]))
